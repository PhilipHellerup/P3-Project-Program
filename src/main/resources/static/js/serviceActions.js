...
export function attachServiceActions() {
    ...
    document.querySelectorAll('#serviceTable .delete-btn').forEach(button => {
      ...
      // the table row of which the desired service to delete will be in.
      const row = e.target.closest('tr');

      // Retrieve the service ID stored as a data attribute on the row
  ...
  document.querySelectorAll('#serviceTable .edit-btn').forEach(button => {
    ...
    button.addEventListener('click', async (e) => {
      ...
      const cells = Array.from(row.querySelectorAll('td[data-field]')).filter(cell => cell.dataset.field !== 'id');
      ...
      if (isEditing) {
        ...
        try {
          await refreshRowFromServer(row, serviceId);
        } catch (err) {
          console.error('Failed to refresh service row:', err);
        }

        // Clean up any temporary data
        delete row.dataset.updatedService;
      }
    });
  });
  ...
  /** @param {string} serviceId ID of the product to update
...
async function fetchService(serviceId) {
  const response = await fetch(`/api/services/${serviceId}`, { method: 'GET' });
  await handleFetchErrors(response);
  return await response.json();
}

/* --- Helper: Refresh row from server (or fallback to last PUT response) --- */
async function refreshRowFromServer(row, serviceId) {
  let service = null;
  try {
    service = await fetchService(serviceId);
  } catch (err) {
    // Fallback to the last PUT response stored on the row
    if (row.dataset.updatedService) {
      try {
        service = JSON.parse(row.dataset.updatedService);
      } catch (_) {
        /* ignore */
      }
    }
  ...
  if (!service) return; // nothing to refresh

  // Update each cell with authoritative values
  const fields = ['name', 'duration', 'price'];
  fields.forEach((field) => {
    const cell = row.querySelector(`td[data-field="${field}"]`);
    if (!cell) return;
    if (field === 'price') {
      cell.textContent = formatPrice(service.price);
    } else {
      cell.textContent = service[field] != null ? String(service[field]) : '';
    }
  });
}
